...
For example binary tree shown below is considered as CORRECT
    2
   / \
  2  2
...
def is_key_greater_root(key, root_key):
    return key > root_key
...
def is_bst(tree):
    is_left_bst = walk_decorator(tree, is_key_greater_root)(tree[0][1])
    if not is_left_bst:
        return False
